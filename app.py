...
with col_a:
    ...
    source_input = st.text_input("Source (edit if needed)", value=inferred_source)
...
def fuzzy_match_source(name: str, source_bias: dict) -> Tuple[Optional[str], float]:
    # simple ratio
    best_key, best_score = None, 0.0
    name_l = (name or "").lower().strip()
    for k in source_bias.keys():
        s = 100.0 if k == name_l else 0.0
        if s > best_score:
            best_key, best_score = k, s
    return best_key, best_score

if article_text:
    match_key, score = fuzzy_match_source(source_input, source_bias_map)
    if match_key and score >= 85:
        bias_info = source_bias_map[match_key]
        st.info(f"Matched known source: {match_key} (score {score})")
    elif use_gpt_bias_fallback and OPENAI_API_KEY and OpenAI is not None:
        try:
            client = OpenAI(api_key=OPENAI_API_KEY)
            prompt = f"""
You are a media bias researcher.
Given the outlet name "{source_input}", infer its general political bias family
...
"""
            ...
            st.warning(f"GPT bias fallback failed: {e}")
    else:
        st.info("No strong match found; proceeding with unknown bias.")
...
if article_text:
    ...
    seg_vecs = topic_vecs_from_text(topic_embedder, article_text, chunk_tokens, normalize)
    st.write(f"Generated {len(seg_vecs)} topic vectors.")
    if len(seg_vecs) > 0:
...
if article_text and OPENAI_API_KEY and OpenAI is not None and topic_results:
    try:
        client = OpenAI(api_key=OPENAI_API_KEY)
...
run_btn = st.button("Run Retrieval & Score", disabled=not (article_text and topic_results and (stance_vec is not None)))
if run_btn:
    ...
    if df.empty:
        ...
        st.dataframe(anti_echo_best, use_container_width=True)

        a, b = st.columns(2)
        with a:
            st.markdown("#### Same Topic — Different Source Bias")
            st.dataframe(same_topic_diff_bias, use_container_width=True)
        with b:
            st.markdown("#### Same Topic — Opposite Stance")
            st.dataframe(same_topic_opp_stance, use_container_width=True)
